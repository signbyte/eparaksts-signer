...
// readyz is readiness: the orchestrator is wired and the job store (Redis) is
// reachable. It returns a plain {status} body — never an error envelope — so an
// orchestrator probe gets a uniform readiness signal, and skips the access
// line/span.
func (r *router) readyz(ctx *azugo.Context) {
	ctx.SkipRequestLog()
	if r.Orchestrator() == nil {
		notReady(ctx, "orchestrator")

		return
	}
	if err := r.Orchestrator().Store().Ping(ctx); err != nil {
		notReady(ctx, "job store")
